...
// TODO: make getters and setters type-safe
// TODO: add handling for converting to/from generic java data
...
public class Data {
    ...
    private static int getInt(byte[] buf, int ofs) {
        return (int) ((0xFF & (int) buf[ofs + 0]) << 24
                ...
                | (0xFF & (int) buf[ofs + 2]) << 8 | (0xFF & (int) buf[ofs + 3]) << 0);
    }
     ...
     * @throws IOException
     */
    private static void writeInt(ByteArrayOutputStream os, int data)
            throws IOException {
        byte[] bytes = new byte[4];
        setInt(bytes, 0, data);
        os.write(bytes);
    ...
    // }
    // public static void writeWord(ByteArrayOutputStream os, long data) throws
    // IOException {
    // byte[] bytes = new byte[4];
    ...
    // }
    // public static void writeLong(ByteArrayOutputStream os, long data) throws
    // IOException {
    // byte[] bytes = new byte[8];
    ...
    // }
    // public static void writeTime(ByteArrayOutputStream os, byte[] data)
    // throws IOException {
    // byte[] bytes = new byte[16];
    ...
    // public static double readDouble(ByteArrayInputStream is) throws
    // IOException {
    // byte[] bytes = new byte[8];
    // is.read(bytes, 0, 8);
    // return getDouble(bytes, 0);
    // }
    // public static void writeDouble(ByteArrayOutputStream os, double data)
    // throws IOException {
    // byte[] bytes = new byte[8];
    ...
    // public static Complex readComplex(ByteArrayInputStream is) throws
    // IOException {
    // byte[] bytes = new byte[16];
    // is.read(bytes, 0, 8);
    // return getComplex(bytes, 0);
    // }
    // public static void writeComplex(ByteArrayOutputStream os, Complex data)
    // throws IOException {
    // byte[] bytes = new byte[16];
    ...
    private void flatten(ByteArrayOutputStream os, Type type, byte[] buf,
            int ofs, Vector<byte[]> heap) throws IOException {
        int i;
        switch (type.getCode()) {
    ...
    private static void unflatten(ByteArrayInputStream is, Type type,
        ...
        switch (type.getCode()) {
            case '_':
                break; // do nothing for empty data
            case 'b':
                is.read(buf, ofs, 1);
                break;
            case 'i':
    ...
    public String pretty() {
        ...
        switch (type.getCode()) {
            ...
            case 'E':
                return "Error(" + Integer.toString(getCode()) + ", "
                        + getMessage() + ")";
    ...
    public int getCode(int... indices) {
        Cluster2<byte[], Integer> pos = getOffset(indices);
        return getInt(pos.get0(), pos.get1());
    }

    public String getMessage(int... indices) {
        Cluster2<byte[], Integer> pos = getOffset(indices);
        int index = getInt(pos.get0(), pos.get1() + 4);
        try {
    ...
    public Data getPayload(int... indices) {
        Type type = getSubtype(indices);
        Cluster2<byte[], Integer> pos = getOffset(indices);
        return new Data(type.getSubtype(0), pos.get0(), pos.get1() + 8, heap);
    }

    public Data setError(int data, int... indices) {
        Cluster2<byte[], Integer> pos = getOffset(indices);
        setTime(pos.get0(), pos.get1(), data);
        return this;
    }
