...
public class Connection {
    ...
    public static void main(String[] args) {
        try {
            ...
            long target = 1;
            Record[] records = {};
            Packet packet = new Packet(new Context(1234, 5678), target,
                    request, records);
            ...
            os.writePacket(new Context(0, 1), target, request, new Record(0,
                    data));
            ...
            for (int i = 0; i < 1000; i++) {
                os.writePacket(new Context(0, 1), hydrantID, request,
                        new Record(0));
                packet = is.readPacket();
                System.out.println("got packet: " + packet.toString());
            }
            ...
            System.out
                    .println("getting random data, make pretty, but don't print...");
            start = System.currentTimeMillis();
            for (int i = 0; i < 1000; i++) {
                os.writePacket(new Context(0, 1), hydrantID, request,
                        new Record(0));
